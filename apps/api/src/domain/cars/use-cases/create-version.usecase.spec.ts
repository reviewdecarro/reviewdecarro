...
describe("CreateVersionUseCase", () => {
  ...
  beforeEach(() => {
    sut = new CreateVersionUseCase(mockBrandsRepository, mockModelsRepository, mockVersionsRepository);
    jest.clearAllMocks();
  });

  const input = { year: 2024, versionName: "Track", engine: "1.0 MPI", transmission: "Manual", slug: "2024-track" };
  ...
  it("should create and return version response DTO", async () => {
    ...
    mockVersionsRepository.create.mockResolvedValue(
      new CarVersionEntity({ id: "v-1", modelId: "m-1", year: 2024, versionName: "Track", engine: "1.0 MPI", transmission: "Manual", slug: "2024-track", createdAt: new Date() }),
    );
  ...
  it("should throw BadRequestError when brand not found", async () => {
    ...
    await expect(sut.execute("nonexistent", "polo", input)).rejects.toThrow(BadRequestError);
    await expect(sut.execute("nonexistent", "polo", input)).rejects.toThrow("Brand not found");
  });
  ...
  it("should throw BadRequestError when model not found", async () => {
    ...
    await expect(sut.execute("volkswagen", "nonexistent", input)).rejects.toThrow(BadRequestError);
    await expect(sut.execute("volkswagen", "nonexistent", input)).rejects.toThrow("Model not found");
  });
  ...
  it("should throw BadRequestError when version slug already exists", async () => {
    ...
    mockVersionsRepository.findBySlug.mockResolvedValue(
      new CarVersionEntity({ id: "existing", modelId: "m-1", year: 2024, versionName: "Track", engine: null, transmission: null, slug: "2024-track", createdAt: new Date() }),
    );

    await expect(sut.execute("volkswagen", "polo", input)).rejects.toThrow(BadRequestError);
    await expect(sut.execute("volkswagen", "polo", input)).rejects.toThrow("Version slug already exists");
  });
});
