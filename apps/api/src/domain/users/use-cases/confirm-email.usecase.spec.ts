...
describe("ConfirmEmailUseCase", () => {
  ...
  beforeEach(() => {
    ...
    rolesRepository.items.push(
      new RoleEntity({ id: "role-user", name: "user", createdAt: new Date(), updatedAt: new Date() }),
    );
  ...
  async function seedUserAndToken(overrides?: { expiresDate?: Date }) {
    ...
    const token = await userTokensRepository.create({
      ...
      expiresDate: overrides?.expiresDate ?? new Date(Date.now() + 60 * 60 * 1000),
    });
  ...
  it("should assign the user role when confirming the email", async () => {
    ...
    expect(rolesRepository.assignments[0]).toEqual({ userId: "user-1", roleId: "role-user" });
  });
  ...
  it("should throw BadRequestError if user role is not configured", async () => {
    ...
    await expect(sut.execute({ token: token.refreshToken })).rejects.toThrow(BadRequestError);
  ...
  it("should throw BadRequestError if token does not exist", async () => {
    await expect(sut.execute({ token: "missing" })).rejects.toThrow(BadRequestError);
    await expect(sut.execute({ token: "missing" })).rejects.toThrow("Token inválido.");
  });

  it("should throw BadRequestError if token is expired", async () => {
    const token = await seedUserAndToken({ expiresDate: new Date(Date.now() - 1000) });

    await expect(sut.execute({ token: token.refreshToken })).rejects.toThrow("Token expirado.");
